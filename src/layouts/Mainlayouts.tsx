...
import { Layout, Menu, Grid, Dropdown, Avatar } from "antd"; // Добавили Grid, Dropdown, Avatar
import {
  DashboardOutlined,
...
export default function MainLayout() {
  ...
  const userMenuItems = [
    { key: "profile", icon: <UserOutlined />, label: user?.name || t('common.user', 'User'), disabled: true },
    { type: "divider" as const },
    { key: "logout", icon: <LogoutOutlined />, label: t('common.logout', 'Log out'), onClick: handleLogout, danger: true }
  ];
    ...
    <Layout style={{ minHeight: "100vh" }}>
      ...
      <Layout>
        <Header style={{
          ...
          {!isMobile && (
            <div onClick={() => setIsOnline(!isOnline)} style={{ cursor: "pointer", display: "flex", alignItems: "center", gap: "8px", background: isDark ? "#1f1f1f" : "#f5f5f5", padding: "6px 14px", borderRadius: "4px", border: `1px solid ${isDark ? "#303030" : "#e8e8e8"}` }}>
          ...
          <Dropdown menu={{ items: userMenuItems }} trigger={['click']} placement="bottomRight">
            <div style={{ cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '8px', padding: '4px 8px', borderRadius: '4px', background: isDark ? "#1f1f1f" : "#f5f5f5", border: `1px solid ${isDark ? "#303030" : "#e8e8e8"}` }}>
        ...
        <Content style={{ margin: isMobile ? "8px" : "24px 16px", padding: isMobile ? 12 : 24, background: isDark ? "#141414" : "#ffffff", borderRadius: 8, transition: "all 0.3s" }}>
          <Outlet />
        </Content>
      </Layout>
    </Layout>
